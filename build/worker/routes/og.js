...
function buildVerdictSvg(verdict, confidence, scam_type, counter) {
...
function buildAlertSvg(title, description) {
...
og.get('/og/image', async (c) => {
    ...
    if (!_ogq.success)
        return c.json({ error: { code: 'VALIDATION_ERROR', message: _ogq.error.issues.map((i) => i.message).join('; ') } }, 400);
    const rawVerdict = _ogq.data.verdict || 'suspicious';
    const verdict = ['phishing', 'suspicious', 'likely_safe'].includes(rawVerdict)
        ? rawVerdict
        : 'suspicious';
...
og.get('/og/alert', (c) => {
    ...
    if (!_alq.success)
        return c.json({ error: { code: 'VALIDATION_ERROR', message: _alq.error.issues.map((i) => i.message).join('; ') } }, 400);
...
og.get('/og/:type', (c) => {
    ...
    if (type === 'verdict') {
        ...
        return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Tip OG invalid. Folositi: verdict sau alert.' }, request_id: rid }, 400);
    }
