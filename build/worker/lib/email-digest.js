...
export function buildEmailHtml(digest, email, baseUrl) {
...
export async function getSubscribers(cache) {
...
export async function addSubscriber(cache, email) {
...
export async function removeSubscriber(cache, email) {
    const list = await getSubscribers(cache);
    const normalized = email.toLowerCase().trim();
...
// ── Main function ─────────────────────────────────────────────────────────────
export async function sendDigestEmail(env, digest) {
    if (!env.CACHE) {
