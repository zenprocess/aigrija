...
export async function generateWeeklyDigestLegacy(env) {
    if (!env.ADMIN_DB)
        return null;
    const campaign = await env.ADMIN_DB.prepare(`SELECT * FROM campaigns WHERE created_at > datetime('now', '-7 days')
...
function pickTips(count, weekYear) {
...
export function weekLabel(date = new Date()) {
    ...
    const fmtDay = (d) => `${d.getDate()} ${RO_MONTHS[d.getMonth()]}`;
    return `${fmtDay(monday)} - ${fmtDay(sunday)} ${sunday.getFullYear()}`;
}
// ─── Main aggregation function ────────────────────────────────────────────────
export async function generateWeeklyDigest(env) {
    ...
    let topScams = [];
    try {
        const rows = await env.DB.prepare(`SELECT id, title, source_url, severity, created_at
       FROM campaigns
       WHERE created_at >= ?
       ORDER BY CASE severity WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END,
       created_at DESC
       LIMIT 10`).bind(sevenDaysAgo).all();
    ...
    const blogPosts = [];
    try {
        const blogRows = await env.DB.prepare(`SELECT id, title, updated_at
       FROM campaigns
       WHERE draft_status = 'published' AND updated_at >= ?
       ORDER BY updated_at DESC
       LIMIT 3`).bind(sevenDaysAgo).all();
