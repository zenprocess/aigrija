...
test.describe('Blog — /blog list page', () => {
...
test.describe('Blog — article detail page', () => {
  test('first article link navigates to detail page', async ({ page }) => {
    ...
    const articleLink = page.locator('article a, .post a, a[href*="/blog/"]').first();
  ...
  test('direct GET /blog/:slug returns 200 or 404', async ({ request }) => {
    // Try a common slug pattern
    const res = await request.get('/blog/test-article');
...
test.describe('Blog — navigation', () => {
  ...
  test('back link on article page if present', async ({ page }) => {
    ...
    const backLink = page.locator('a[href="/blog"], [data-testid="back-btn"], a:has-text("napoi"), a:has-text("Blog")').first();
