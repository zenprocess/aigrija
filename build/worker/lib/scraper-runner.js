...
export async function runScraper(source, env) {
    ...
    for (const item of items) {
        ...
        try {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 0, ?)`).bind(id, data.title, data.slug, data.source, data.sourceUrl, data.publishedAt ?? null, data.bodyText ?? null, data.threatType ?? null, data.affectedBrands ? JSON.stringify(data.affectedBrands) : null, data.iocs ? JSON.stringify(data.iocs) : null, data.severity ?? null, new Date().toISOString()).run();
            campaignId = id;
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
...
async function logRun(env, sourceName, itemsFound, itemsNew, errors, runAt) {
    try {
        await env.DB.prepare(`INSERT INTO scraper_runs (id, source, items_found, items_new, errors, run_at)
       VALUES (?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), sourceName, itemsFound, itemsNew, errors.length ? JSON.stringify(errors) : null, runAt).run();
    }
    catch (err) {
