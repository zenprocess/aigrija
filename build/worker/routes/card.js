...
card.get('/card/:hash/image', async (c) => {
    ...
    if (!_hashResult.success) {
        ...
        return c.json({ error: { code: 'VALIDATION_ERROR', message: _hashResult.error.issues.map((i) => i.message).join('; ') }, request_id: rid }, 400);
    }
...
card.get('/card/:hash', async (c) => {
    ...
    if (!_hashResult2.success) {
        const rid = (c.get('requestId')) || 'unknown';
        return c.json({ error: { code: 'VALIDATION_ERROR', message: _hashResult2.error.issues.map((i) => i.message).join('; ') }, request_id: rid }, 400);
    }
    ...
    let meta = { verdict: 'safe', scam_type: 'unknown' };
    if (raw) {
        try {
            meta = JSON.parse(raw);
        }
