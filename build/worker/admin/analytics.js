...
analytics.get('/admin/analytics', async (c) => {
    ...
    let severityRows = { results: [] };
    let monthRows = { results: [] };
    try {
    ...
    const severityData = JSON.stringify(severityRows.results.map(r => ({
        label: r.severity,
        count: r.count,
    })));
    const monthData = JSON.stringify(monthRows.results.map(r => ({
        label: r.month,
        count: r.count,
    })));
