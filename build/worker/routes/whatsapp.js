...
function formatAnalysisReply(result, urlFlags, isForwarded, cardUrl) {
...
async function sendWhatsAppInteractive(accessToken, phoneNumberId, to, bodyText, buttons) {
    try {
        await fetch(`https://graph.facebook.com/v21.0/${phoneNumberId}/messages`, {
            method: 'POST',
            headers: {
...
async function markMessageRead(accessToken, phoneNumberId, messageId) {
    try {
        await fetch(`https://graph.facebook.com/v21.0/${phoneNumberId}/messages`, {
            method: 'POST',
            headers: {
...
async function verifyHmacSha256(secret, rawBody, signatureHeader) {
        ...
        for (const change of entry.changes ?? []) {
            ...
            for (const message of value.messages ?? []) {
                ...
                if (upperText === 'START') {
                    ...
                    await sendWhatsAppMessage(accessToken, phoneNumberId, from, 'Bine ai venit la ai-grija.ro! Prin trimiterea acestui mesaj ti-ai dat consimtamantul pentru prelucrarea datelor (GDPR). Trimite STERGE oricand pentru a-ti sterge datele.');
                    continue;
                }
                // GDPR: opt-out / delete
                if (upperText === 'STERGE' || upperText === 'STOP') {
                    await revokeConsent(c.env, 'wa', from);
                    await sendWhatsAppMessage(accessToken, phoneNumberId, from, 'Datele tale au fost sterse. Nu vei mai primi alerte de la ai-grija.ro.');
                    continue;
                }
                ...
                if (!rl.allowed) {
                    await sendWhatsAppMessage(accessToken, phoneNumberId, from, 'Ai atins limita de 50 verificări/oră. Te rugăm să încerci din nou mai târziu.');
                    continue;
                }
                ...
                try {
                    ...
                    await sendWhatsAppMessage(accessToken, phoneNumberId, from, 'A aparut o eroare la analiza. Te rugam sa incerci din nou.');
                    continue;
                }
                const urlFlags = [];
