...
async function sendMessage(token, chatId, text, inlineKeyboard) {
    const body = {
        chat_id: chatId,
        text,
        parse_mode: 'HTML',
...
async function answerInlineQuery(token, inlineQueryId, results) {
    try {
...
function buildVerdictKeyboard(hash, baseUrl) {
    return [
        [
...
function verdictLabel(verdict) {
    if (verdict === 'phishing')
        return 'PHISHING DETECTAT';
    if (verdict === 'suspicious')
        return 'MESAJ SUSPECT';
    return 'PROBABIL SIGUR';
}
function formatAnalysisReply(result, urlFlags) {
...
telegram.post('/webhook/telegram', async (c) => {
    ...
    if (text === '/start' || text.startsWith('/start ')) {
        ...
        await sendMessage(token, chatId, 'Bine ai venit la ai-grija.ro Bot! Trimite-mi orice mesaj suspect si il verific instant. Prin utilizarea acestui bot iti dai consimtamantul pentru prelucrarea datelor (GDPR). Foloseste /sterge pentru a-ti sterge datele. Foloseste /help pentru instructiuni complete.');
        return c.json({ ok: true });
    }
    if (text === '/sterge' || text.startsWith('/sterge ')) {
        await revokeConsent(c.env, 'tg', String(chatId));
        await sendMessage(token, chatId, 'Datele tale au fost sterse. Nu vei mai primi alerte.');
        return c.json({ ok: true });
    }
    if (text === '/help' || text.startsWith('/help ')) {
        await sendMessage(token, chatId, 'Cum folosesti ai-grija.ro Bot: Trimite textul unui mesaj suspect. Limita: 50 verificari/ora per utilizator. Raporteaza fraude la DNSC: 1911');
        return c.json({ ok: true });
    }
    if (text === '/alerte' || text.startsWith('/alerte ')) {
        const activeCampaigns = CAMPAIGNS.filter(camp => camp.status === 'active');
        const severityEmoji = (s) => s === 'critical' ? '🔴' : s === 'high' ? '🟠' : '🟡';
        const lines = ['Campanii active de phishing:', ''];
    ...
    if (text === '/about' || text.startsWith('/about ')) {
        await sendMessage(token, chatId, 'Despre ai-grija.ro: proiect civic gratuit creat de Zen Labs. Verifica mesaje suspecte, raporteaza la autoritati, protejeaza-ti familia. ai-grija.ro | contact@ai-grija.ro');
        return c.json({ ok: true });
    }
    // ── Update last_active ───────────────────────────────────────────────────
    ...
    if (!rlAllowed) {
        await sendMessage(token, chatId, 'Ai atins limita de 50 verificari/ora. Te rugam sa incerci din nou mai tarziu.');
        return c.json({ ok: true });
    }
    // ── Classify ──────────────────────────────────────────────────────────────
        ...
        for (const url of urls) {
            try {
                ...
                if (analysis.is_suspicious) {
                    urlFlags.push(...analysis.flags.map((f) => `[URL] ${f}`));
                }
            }
            catch {
