...
const BATCH_LIMIT = 5;
async function runDraftGeneration(env) {
    structuredLog('info', 'cron_draft_generation_start', { stage: 'cron' });
    const pendingRows = await env.DB.prepare(`SELECT * FROM campaigns WHERE draft_status = 'pending' LIMIT ?`).bind(BATCH_LIMIT).all();
...
async function runSanityPublish(env) {
    structuredLog('info', 'cron_sanity_publish_start', { stage: 'cron' });
    const generatedRows = await env.DB.prepare(`SELECT * FROM campaigns WHERE draft_status = 'generated' LIMIT ?`).bind(BATCH_LIMIT).all();
    ...
    for (const campaign of generated) {
        ...
        if (publishedCount > 0) {
            try {
                await env.DB.prepare(`UPDATE campaigns SET draft_status = 'published', updated_at = ? WHERE id = ?`).bind(new Date().toISOString(), campaign.id).run();
