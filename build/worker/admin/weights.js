...
import { getWeights, saveWeights, getWeightHistory, DEFAULT_WEIGHTS, scoreUrlWithWeights } from '../lib/weights';
const WEIGHT_LABELS = {
...
function weightRow(key, current, def) {
    return `
...
function weightsPage(weights, history, email) {
...
export const weightsAdmin = new Hono();
...
weightsAdmin.get('/test', async (c) => {
    ...
    if (!url)
        return c.html('<p class="text-gray-400 text-sm">Introduti un URL pentru testare.</p>');
    ...
    const breakdownRows = Object.entries(withProposed.breakdown).map(([k, v]) => `<tr><td class="py-1 px-2 text-xs text-gray-600">${WEIGHT_LABELS[k] ?? k}</td><td class="py-1 px-2 text-xs font-mono">${v.toFixed(2)}</td></tr>`).join('') || '<tr><td colspan="2" class="text-gray-400 text-xs py-1 px-2">Niciun flag activ</td></tr>';
    return c.html(`
