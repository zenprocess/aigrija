...
async function getFlagStatus(kv, name) {
    try {
...
async function getRateLimit(kv, endpoint, def) {
    try {
...
async function getCbState(kv, name) {
    try {
...
async function configPage(kv, email) {
    ...
    const cbStateClass = (s) => s === 'closed' ? 'bg-green-100 text-green-700' :
...
export const configAdmin = new Hono();
...
configAdmin.post('/flag', async (c) => {
    ...
    if (!name || !validNames.includes(name))
        return c.json({ error: 'Invalid flag name' }, 400);
...
configAdmin.post('/ratelimit', async (c) => {
    const { endpoint, limit } = await c.req.json();
...
configAdmin.post('/circuit-reset', async (c) => {
    ...
    if (!name || !CIRCUIT_BREAKERS.includes(name))
        return c.json({ error: 'Invalid circuit breaker name' }, 400);
