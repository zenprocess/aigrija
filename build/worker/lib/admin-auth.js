...
async function fetchCFAccessCerts(teamDomain, kv) {
...
async function verifyJWT(token, teamDomain, kv) {
    ...
    try {
        ...
        cryptoKey = await crypto.subtle.importKey('spki', derBytes, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']);
    }
    catch {
        // Try EC key fallback
        try {
            const derBytes = toUint8Array(base64urlDecode(pemBody));
            cryptoKey = await crypto.subtle.importKey('spki', derBytes, { name: 'ECDSA', namedCurve: 'P-256' }, false, ['verify']);
        }
        catch {
            return null;
        }
    ...
    try {
        ...
        try {
            valid = await crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, cryptoKey, signature, signedData);
        }
        catch {
            return null;
        }
