...
function buildUserMessage(campaign) {
...
export async function generateDraft(campaignId, env) {
    ...
    try {
        const result = await env.AI.run(DRAFT_MODEL, { messages, max_tokens: 1500 });
        const content = result?.response || '';
        await env.DB.prepare(`UPDATE campaigns SET draft_content = ?, draft_status = 'generated', updated_at = ? WHERE id = ?`).bind(content, new Date().toISOString(), campaignId).run();
...
export async function generateMultipleDrafts(campaignId, env) {
    const campaign = await fetchCampaign(campaignId, env);
    if (!campaign) {
    ...
    await env.DB.prepare(`UPDATE campaigns SET draft_content = ?, draft_status = 'generated', updated_at = ? WHERE id = ?`).bind(draftsJson, new Date().toISOString(), campaignId).run();
    structuredLog('info', '[draft-generator] Multi-draft generated', { campaignId, variants: drafts.length });
}
async function fetchCampaign(campaignId, env) {
    const row = await env.DB.prepare(`SELECT * FROM campaigns WHERE id = ?`).bind(campaignId).first();
    return row || null;
}
export async function generateStandalonePost(env) {
    ...
    let topic = '';
    try {
        const topicResult = await env.AI.run(DRAFT_MODEL, {
            messages: [
    ...
    let articleContent = '';
    try {
        const articleResult = await env.AI.run(DRAFT_MODEL, {
            messages: [
    ...
    try {
        await env.DB.prepare(`INSERT INTO campaigns (id, title, source, draft_status, draft_content, severity, threat_type, created_at, updated_at)
       VALUES (?, ?, 'ai-generated', 'generated', ?, 'medium', ?, ?, ?)`).bind(id, topic, articleContent, category, now, now).run();
