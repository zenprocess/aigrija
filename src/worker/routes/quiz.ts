...
quiz.get('/api/quiz', async (c) => {
  ...
  if (!rl.allowed) {
    return c.json(
      { error: { code: 'RATE_LIMITED', message: 'Prea multe cereri. Reîncercați mai târziu.' } },
      429,
    );
  }
...
quiz.post('/api/quiz/check', async (c) => {
  ...
  let body: CheckBody;
  try {
    body = await c.req.json<CheckBody>();
  } catch {
  ...
  if (!questionId) {
    return c.json(
      { error: { code: 'BAD_REQUEST', message: 'Câmpul questionId este obligatoriu.' } },
      400,
    );
  }
  ...
  if (q.type === 'ce_faci_daca') {
    ...
    const noWrongChosen = userChoices.every((t) =>
      q.options.find((o) => o.text === t)?.correct === true,
    );
