...
report.get('/api/report/:type', async (c) => {
  try {
    ...
    structuredLog('error', 'report_handler_error', { error: String(err) });
    return c.json({ error: { code: 'INTERNAL_ERROR', message: 'A aparut o eroare la generarea raportului.' } }, 500);
  }
});
