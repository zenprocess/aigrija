...
import { z } from 'zod';
import { CheckRequestSchema, formatZodError } from '../lib/schemas';
...
function buildShareSvg(verdict, confidence, scam_type) {
    const styles = {
...
export class CheckEndpoint extends OpenAPIRoute {
    ...
    async handle(c) {
        ...
        if (gibberishCheck.gibberish) {
            return c.json({ error: { code: 'GIBBERISH_INPUT', message: 'Textul introdus nu pare a fi un mesaj valid.', request_id: rid } }, 400);
        }
        // Load feature flags
        ...
        const urlAnalysis = body.url
            ? await analyzeUrl(body.url, safeBrowsingEnabled ? c.env.GOOGLE_SAFE_BROWSING_KEY : undefined, c.env.VIRUSTOTAL_API_KEY, c.env.CACHE, c.env.URLHAUS_AUTH_KEY)
            : undefined;
        ...
        if (classification.verdict === 'phishing' || classification.verdict === 'suspicious') {
            ...
            c.executionCtx.waitUntil(storeCommunityReport(c.env.CACHE, rid, {
                url: body.url,
                text: body.text,
                verdict: classification.verdict,
        ...
        const svgContent = buildShareSvg(classification.verdict, classification.confidence, classification.scam_type);
        ...
        c.executionCtx.waitUntil(prependFeedEntry(c.env.CACHE, {
            verdict: classification.verdict,
            scam_type: classification.scam_type,
            timestamp: Date.now(),
        }));
