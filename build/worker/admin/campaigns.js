...
// ---- JSON API routes -------------------------------------------------------
export const campaignApiRoutes = new Hono();
campaignApiRoutes.get('/list', async (c) => {
    const { page = '1', limit = '20', q = '', source = '', severity = '', status = '' } = Object.fromEntries(Array.from(new URL(c.req.url).searchParams));
    ...
    let rows = { results: [] };
    try {
        countRow = await c.env.DB.prepare(`SELECT COUNT(*) as total FROM campaigns ${where}`).bind(...params).first();
        rows = await c.env.DB.prepare(`SELECT id, title, slug, source, severity, draft_status, published_at, created_at
       FROM campaigns ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(...params, limitNum, offset).all();
    }
    catch (err) {
...
campaignApiRoutes.get('/:id', async (c) => {
    let row = null;
    try {
        row = await c.env.DB.prepare('SELECT * FROM campaigns WHERE id = ?').bind(c.req.param('id')).first();
    }
    catch (err) {
...
campaignApiRoutes.put('/:id', async (c) => {
    ...
    if (body.threat_type !== undefined) {
        updates.push('threat_type = ?');
        vals.push(body.threat_type);
    }
    if (body.affected_brands !== undefined) {
        updates.push('affected_brands = ?');
        vals.push(JSON.stringify(body.affected_brands));
    }
    if (body.iocs !== undefined) {
        updates.push('iocs = ?');
        vals.push(JSON.stringify(body.iocs));
    }
    if (body.archived !== undefined) {
        updates.push('archived = ?');
        vals.push(body.archived);
    }
    if (body.draft_status !== undefined) {
        updates.push('draft_status = ?');
        vals.push(body.draft_status);
    }
    if (!updates.length)
        return c.json({ error: 'No fields to update' }, 400);
    vals.push(id);
    try {
        await c.env.DB.prepare(`UPDATE campaigns SET ${updates.join(', ')} WHERE id = ?`).bind(...vals).run();
    }
    catch (err) {
...
campaignApiRoutes.post('/create', async (c) => {
    ...
    try {
        await c.env.DB.prepare(`INSERT INTO campaigns (id, title, slug, source, source_url, threat_type, severity, draft_status, archived, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', 0, ?)`).bind(id, body.title, slug, body.source ?? 'manual', body.source_url ?? null, body.threat_type ?? null, body.severity ?? null, new Date().toISOString()).run();
    }
    catch (err) {
...
campaignRoutes.get('/', async (c) => {
    ...
    try {
        countRow = await c.env.DB.prepare(`SELECT COUNT(*) as total FROM campaigns ${where}`).bind(...params).first();
        rows = await c.env.DB.prepare(`SELECT id, title, slug, source, severity, draft_status, published_at, created_at
       FROM campaigns ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(...params, limit, offset).all();
    }
    catch (err) {
...
campaignRoutes.get('/:id', async (c) => {
    if (c.req.param('id') === 'new')
        return c.redirect('/campanii/new');
    const email = c.get('adminEmail');
    let row = null;
    try {
        row = await c.env.DB.prepare('SELECT * FROM campaigns WHERE id = ?').bind(c.req.param('id')).first();
    ...
    if (!row)
        return c.html(adminLayout('Not Found', '<p>Campanie negasita.</p>', 'campanii', email), 404);
    let brands = [];
    let iocs = [];
    try {
        brands = row.affected_brands ? JSON.parse(row.affected_brands) : [];
    }
    catch {
        brands = [];
    }
...
scraperRoutes.get('/', async (c) => {
    ...
    let rows = { results: [] };
    try {
        rows = await c.env.DB.prepare('SELECT * FROM scraper_runs ORDER BY run_at DESC LIMIT 50').all();
    }
    catch (err) {
