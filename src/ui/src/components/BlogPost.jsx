      ...
      {body.map((block, idx) => {
        if (block._type === 'block') {
          ...
          if (style === 'blockquote') return (
            <blockquote key={idx} className="border-l-4 border-blue-500 pl-4 my-4 text-gray-300 italic">{inner}</blockquote>
          );
        ...
        if (block._type === 'image') {
          return (
            <figure key={idx} className="my-6">
              <img
                src={block.asset?.url || block.url}
                alt={block.alt || ''}
                className="w-full rounded-xl"
              />
              {block.caption && (
                <figcaption className="text-sm text-gray-500 text-center mt-2">{block.caption}</figcaption>
              )}
            </figure>
          );
        }
...
export default function BlogPost({ slug }) {
  ...
  const handleTts = () => {
    ...
    const textContent = typeof post.body === 'string'
      ? post.body.replace(/<[^>]+>/g, ' ')
        ...
        {!loading && !error && post && (
          <article>
            ...
            {post.categories && post.categories.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-4">
                {post.categories.map((cat) => (
                  <span
                    key={cat}
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/20 text-blue-300 border border-blue-500/30"
                  >
            ...
            <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-white/10">
              <button
                data-testid="blog-post-share-btn"
                onClick={handleShare}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-white/20 text-gray-300 hover:text-white hover:border-white/40 transition-colors text-sm font-medium"
              >
              ...
              <button
                data-testid="blog-post-tts-btn"
                onClick={handleTts}
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border transition-colors text-sm font-medium ${
                  ttsPlaying
              ...
              <div className="mt-12">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {related.map((rel) => (
                    <a
                      key={rel.slug}
                      data-testid={`blog-related-${rel.slug}`}
                      href={`#/blog/${rel.slug}`}
                      className="glass-card border border-white/10 rounded-xl p-4 hover:border-blue-500/40 transition-colors group"
                    >
