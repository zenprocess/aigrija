...
async function aiTranslate(ai, targetLang, sourceText) {
...
const RO_FLAT = flattenKeys(roJson);
async function getLangKeys(kv, lang) {
    const base = lang === 'ro' ? { ...RO_FLAT } : {};
    try {
...
async function getKvOverrides(kv, lang) {
...
function translationsPage(activeLang, keys, overrides, email, search = '') {
    ...
    const filteredEntries = Object.entries(RO_FLAT).filter(([k]) => !search || k.toLowerCase().includes(search.toLowerCase()));
...
export const translationsAdmin = new Hono();
translationsAdmin.get('/', async (c) => {
    const email = c.get('adminEmail');
    const langParam = c.req.query('lang') ?? 'ro';
    const lang = SUPPORTED_LANGS.includes(langParam) ? langParam : 'ro';
...
translationsAdmin.delete('/api/key', async (c) => {
    const { lang, key } = await c.req.json();
    if (!lang || !key)
        return c.json({ error: 'Missing fields' }, 400);
...
translationsAdmin.post('/api/import', async (c) => {
    const { lang, data } = await c.req.json();
...
translationsAdmin.post('/api/auto-translate', async (c) => {
    const { lang, key, sourceText } = await c.req.json();
    if (!lang || !key || !sourceText)
        return c.json({ error: 'Missing fields' }, 400);
    if (!SUPPORTED_LANGS.includes(lang) || lang === 'ro')
        return c.json({ error: 'Invalid lang' }, 400);
    if (sourceText.length > 2000)
        return c.json({ error: 'sourceText exceeds 2000 character limit' }, 400);
    ...
    if (perMinuteCount >= 10)
        return c.json({ error: 'Rate limit exceeded: max 10 auto-translate requests per minute' }, 429);
    await c.env.CACHE.put(perMinuteKey, String(perMinuteCount + 1), { expirationTtl: 60 });
    const targetLanguage = LANG_FULL_NAMES[lang];
    try {
...
translationsAdmin.post('/api/auto-translate-all', async (c) => {
    ...
    if (!lang || !SUPPORTED_LANGS.includes(lang) || lang === 'ro') {
        return c.json({ error: 'Invalid lang' }, 400);
    }
    const adminId = c.get('adminEmail');
    const perMinuteKey = `ratelimit:translate:${adminId}`;
    ...
    await c.env.CACHE.put(perMinuteKey, String(perMinuteCount + 1), { expirationTtl: 60 });
    const targetLanguage = LANG_FULL_NAMES[lang];
    const existingOverrides = await getKvOverrides(c.env.CACHE, lang);
    let translated = 0;
    let skipped = 0;
    let failed = 0;
    let processed = 0;
