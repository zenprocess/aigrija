...
describe('POST /api/newsletter/subscribe', () => {
  ...
  it('returns 429 when rate limit is exceeded', async () => {
    ...
    const res = await app.fetch(req, makeEnv({ [rlKey('newsletter:9.9.9.9', 3600)]: '1000' }) as any, ctx);
...
describe('POST /api/newsletter/unsubscribe', () => {
  ...
  it('returns 429 when rate limit is exceeded for unsubscribe', async () => {
    ...
    const res = await app.fetch(req, makeEnv({ [rlKey('newsletter:8.8.8.8', 3600)]: '1000' }) as any, ctx);
    expect(res.status).toBe(429);
  });
});
